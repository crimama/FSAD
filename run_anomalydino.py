...
from src.utils import filter_dataset_info, get_dataset_info
...
def validate_data_root(data_root):
    resolved = os.path.abspath(data_root)
    if not os.path.isdir(resolved):
        raise FileNotFoundError(
            f"Dataset root does not exist: {resolved}. "
            "Pass --data_root with a valid dataset directory."
        )
    return resolved
...
def parse_args(argv=None):
    ...
    parser.add_argument("--data_root", type=str, default="../MVTecAD",
                        help="Path to the root directory of the dataset.")
    parser.add_argument("--category", type=str, default=None,
                        help="Optional single category/object to evaluate.")
    ...
    parser.add_argument("--mask_ref_images", default=False, action=argparse.BooleanOptionalAction)
...
def run(args):
    args.data_root = validate_data_root(args.data_root)
    ...
    if args.category is not None:
        print(f"Restricting evaluation to category: {args.category}")
    ...
    objects, object_anomalies, masking_default, rotation_default = filter_dataset_info(
        objects,
        object_anomalies,
        masking_default,
        rotation_default,
    ...
    model = get_model(args.model_name, resolved_device, smaller_edge_size=args.resolution)
    ...
    for shot in list(args.shots):
        ...
        for seed in seeds:
            ...
            if os.path.exists(f"{results_dir}/metrics_seed={seed}.json"):
                ...
                with open(measurements_file, 'w', newline='') as file:
                    ...
                    for object_name in objects:
                        ...
                        anomaly_scores, time_memorybank, time_inference = run_anomaly_detection(
                                                                                ...
                                                                                faiss_on_cpu = args.faiss_on_cpu or resolved_device == "cpu",
                ...
                if skip_eval:
                    continue
                if not (args.eval_clf or args.eval_segm):
                    print(f"Skipping evaluation for seed {seed} because both eval flags are disabled.")
                else:
