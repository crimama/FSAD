...
def run_anomaly_detection(
    ...
    if faiss is None:
        raise ImportError("faiss is required. Install `faiss-cpu` or provide a faiss build with GPU support.")
    ...
    with torch.inference_mode():
        ...
        if not use_faiss_gpu:
            ...
            try:
                ...
                print(f"Warning: failed to initialize GPU FAISS ({exc}). Falling back to CPU FAISS.")
                knn_index = faiss.IndexFlatL2(features_ref.shape[1])
